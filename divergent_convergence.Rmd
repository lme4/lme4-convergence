...
mm <- transform(modeldata.n400,sdiff=c.(sdiff),
                subj=reorder(subj,mean),
                item=reorder(item,mean))
ggplot(mm,aes(mean,subj,colour=c.(sdiff)))+geom_point(alpha=0.5)+
...
```
Note we still get convergence warnings, even with lme4 `r lme4ver`; this means they are probably **not** just false positives (so in this case they are a good thing!)
